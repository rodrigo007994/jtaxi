...
public class GetDriverHistoric {
	...
	public static String getRides(String email){
        ...
        try{
			...
			rs = stmt.executeQuery("SELECT jtaxi_rides.id, CONCAT(jtaxi_users.name, ', ', jtaxi_users.lastname) AS fullname, jtaxi_rides.latitude, jtaxi_rides.longitude, DATE(jtaxi_rides.stamp) AS stamp2, jtaxi_rides.status FROM jtaxi_rides LEFT JOIN jtaxi_users ON jtaxi_rides.client=jtaxi_users.email WHERE jtaxi_rides.driver='"+email+"' AND jtaxi_rides.status!='ESPERANDO TAXI' AND jtaxi_rides.status!='EN CAMINO AL DESTINO' LIMIT 100;");
