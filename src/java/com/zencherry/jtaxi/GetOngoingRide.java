...
public class GetOngoingRide {
	...
	public static String getRide(String email){
        ...
        try{
			...
			rs = stmt.executeQuery("SELECT jtaxi_rides.id, "
					...
					+ "colonia, "
					+ "referencia, "
					+ "CONCAT(jtaxi_users.name, ', ', jtaxi_users.lastname) AS fullname, jtaxi_users.phone, jtaxi_rides.latitude, jtaxi_rides.longitude, round(EXTRACT(EPOCH FROM (now() - jtaxi_rides.stamp))/60) AS stamp2, jtaxi_rides.status FROM jtaxi_rides LEFT JOIN jtaxi_users ON jtaxi_rides.driver=jtaxi_users.email WHERE jtaxi_rides.client='"+email+"' AND (jtaxi_rides.status='ESPERANDO TAXI' OR jtaxi_rides.status='EN CAMINO AL DESTINO')  LIMIT 1;");
			if(rs.next())
			{	
				...
				content+="</td>";
				content+="<td>";
				content+="<button onclick='xhrCancelTaxi()' class='btn btn-primary'>Cancelar Servicio</button>";
				content+="</td>";
				content+="</tr>";
	...
	public static Boolean test(String email){
        ...
        try{
            ...
            conn= DriverManager.getConnection("jdbc:postgresql://localhost:5432/rodrigov_zendb","rodrigov_zen", "4DZ856pn1pTu");
			stmt = conn.createStatement();
			rs = stmt.executeQuery("SELECT jtaxi_rides.id FROM jtaxi_rides WHERE jtaxi_rides.client='"+email+"' AND (jtaxi_rides.status='ESPERANDO TAXI' OR jtaxi_rides.status='EN CAMINO AL DESTINO') ORDER BY jtaxi_rides.id DESC LIMIT 1;");
			if(!rs.next())
			{	
				resultado=false;		
			}
